...
const Highlights = () => {
  ...
  useEffect(() => {
    ...
    return () => {highlightsSection.removeEventListener("scroll", handleScroll);window.removeEventListener("scroll", handleScroll)};
    ...
    function handleScroll() {
      ...
      function handleIntersection(entries) {
        ...
        if (entry.target.id === "highlightsID") setPositionParent(entry.boundingClientRect.top + entry.boundingClientRect.height / 2)
        if (entry.target.id === "imageContainerOne") setPositionImageOne(entry.boundingClientRect.top + entry.boundingClientRect.height)
        if (entry.target.id === "imageContainerTwo") setPositionImageTwo(entry.boundingClientRect.top + entry.boundingClientRect.height)
        if (entry.target.id === "imageContainerThree") setPositionImageThree(entry.boundingClientRect.top + entry.boundingClientRect.height)
  ...
  useEffect(()=>{
      ...
      if (positionImageOne - positionParent < 0) sliderElements[0].classList.remove('isActive'); else sliderElements[0].classList.add('isActive');
      if (sliderElements[1].previousElementSibling.classList.contains('isActive') || positionImageTwo - positionParent < 0) sliderElements[1].classList.remove('isActive'); else if (positionImageTwo - positionParent > 0) sliderElements[1].classList.add('isActive')
      if (sliderElements[1].previousElementSibling.classList.contains('isActive') || sliderElements[2].previousElementSibling.classList.contains('isActive')) sliderElements[2].classList.remove('isActive'); else sliderElements[2].classList.add('isActive')
